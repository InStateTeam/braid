...
class MiximModelConverterV3(val mapper: ObjectMapper) : ModelConverter {
  ...
  override fun resolve(
    ...
    if(schema == null || !(type.type is Class<*>))
      return schema

    val mixinClass = mapper.findMixInClassFor(type.type as Class<*>)
    if (mixinClass == null)
      return schema

    return addMixinSchema(mixinClass,context,schema)
  }

  private fun addMixinSchema(mixin: Class<*>, context: ModelConverterContext, schema: Schema<Any>): Schema<*> {
    ...
    if(schema.name != null)
      context.defineModel(schema.name,schema)
